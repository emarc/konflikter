...
public class LiveConflictView extends Div {
    ...
    @Autowired
    public LiveConflictView(SampleEntityService sampleEntityService) {
                ...
                localUser, topic -> {
                    ...
                    return fieldValues.subscribe(event -> {
                        if ("user".equals(event.getKey())) {
                            ...
                            sampleEntityService.get(this.sampleEntity.getId()).ifPresentOrElse(e -> {
                                this.sampleEntity = e;
                                binder.merge(this.sampleEntity);
                                Notification.show(item.getName() + " made some changes. Please review before saving.");
                            }, () -> {
